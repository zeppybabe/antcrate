...
- **PLAN WRITTEN (complete, self-reviewed, 8 tasks) — but the session hit the 140k gate before it could be copied to master.** Plan file sits at `.claude/worktrees/least-cost-spec/docs/plans/2026-06-11-least-cost-allocation-and-skill-scoping.md` (uncommitted in the worktree; safe on disk). **NEXT session, first three commands:** (1) `cp` that file to `docs/plans/`; (2) `ANTCRATE_COMMIT_PREAPPROVED=1 antcrate --commit antcrate -m "docs(plans): least-cost allocation + skill scoping implementation plan" -- docs/plans/2026-06-11-least-cost-allocation-and-skill-scoping.md`; (3) `antcrate --pp antcrate -y`. Then execute the plan — header mandates subagent-driven-development or executing-plans; user rule says subagent-driven fits (complete upfront design). Build step 1 = `policy.json` + `tests/policy.bats`, RED-first. **Evidence note for the Unit 5 ledger entry when the raise ships: this session is the SECOND Fable run gated at 140k while operating fine** (first: 2026-06-10 >300k clean).
