...
- **BACKLOG REMAINING (medium queue, fresh session recommended):** no-decision-needed: `repoint` + `recover-from-backup` (recovery pair), `ci-core` (`--ci --only=core`), `hook-internal-md-guard`, `obsidian-prune`. USER-DECISION items: retire/decline list (`gateway-guard`/`shellcheck-gate`/`rule-audit`/`session-close` wrapper flags — hooks/skills already enforce these; `subagent-smoke-ping` — obsolete since the 2026-06-06 root-cause), `unnest` semantics (destructive-adjacent, Gateway design), gh bucket (`gh-publish`, `mirror`+2 extensions — public-repo policy), `drive-bundle` (external dep), `commit-patch-mode` (big). Then roadmaps #3 token-limit auto-resume + #6 `--health` (design-heavy, own sessions) → AnyCrate → intel-report (4).
