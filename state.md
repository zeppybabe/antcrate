...
**2026-06-09 (latest) — Loop Engine `--loop` SHIPPED + the relocate was de-facto REVERTED after `~/projects/antcrate` proved EPHEMERAL.**

- **`--loop` is live.** `lib/loop.sh` + `tests/loop.bats` (28 tests) + `bin/antcrate` wiring. Durable objective-loop (state in `~/.antcrate/loops/<id>.json`), 3 hard stops (max-iter/no-progress/budget-proxy), two-key verify (project `--ci` + Claudia `--loop-signoff`), safety-floor precondition (canary + gateway-guard), halt = checkpoint+ledger+quarantine. Composes with Claude Code `/loop` (RESCHEDULE / "LOOP COMPLETE — do not reschedule" contract). **`--ci` PASS @ 480 bats.** Installed to system wrapper (`--install-from-source`). Commits `69f18ff` (engine) + `ab2e683` (wiring) on top of `fcb4a75`. **NOT pushed** (Gateway Law → `antcrate --pp antcrate` when user asks). Spec: `docs/specs/2026-06-09-loop-engine-harness-design.md`. Sub-project #1 of 6 (next: #2 `--costs`, #3 token-limit auto-resume).

- **`~/projects/antcrate` is EPHEMERAL — the 2026-06-06 relocate is effectively reverted.** A session-limit reset mid-build DESTROYED the entire `~/projects/antcrate` tree (incl. `.git` + 8 in-session commits). Recovered: restored the Jun-6 backup tarball to `~/.claude/skills/antcrate` as a REAL directory (replacing the dangling symlink), rebuilt all loop work from conversation context, repointed registry path + `~/.antcrate/config` ANTCRATE_SELFSRC + `~/.claude/settings.json` hook paths (gateway-guard, shellcheck-on-save) off the dead path. **Persistence rule (verified): `~/.claude/**` + `~/.antcrate/**` persist; `~/projects/antcrate` did NOT.** See memory `project_2026_06_09_loop_engine_and_ephemeral_path_loss`.

- **LOOSE ENDS:** (1) `env-guard.sh` hook was an in-session-only file (created after the Jun-6 backup) — LOST; its `settings.json` entry was removed. Rebuild if still wanted. (2) Background agents can't write under `~/.claude` → Cody/Claudia must run FOREGROUND now (the relocate's whole point was to dodge this; it's back). (3) The implementation `plan` doc was not recreated (feature shipped; spec was). (4) Consider whether to re-attempt a relocate to a path that actually persists, or stay at `~/.claude/skills/antcrate` and accept foreground-only agents.
