...
- **NEXT SESSION:** roadmap **#3 token-limit auto-resume**, then #4 agent roles, #5 provisioning, #6 `--health`. Also pending: state.md TRIM (~40k tokens — biggest session-start saving; archive the "Earlier" sections below into the ledger and delete them here).
