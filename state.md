...
- **Session queue (user-approved order):** ✅ (1) persistence insurance → ✅ (2) codebase audit (1 CRITICAL hooks.sh rm fixed via remove-by-rename, 6 minors fixed, AGENTS #16 promoted to live; **new baseline 498 bats, next audit at 598**; ~/CLAUDE.md counter line needs user update) → (3) `--cost`/budget engine (parse `~/.claude/projects/` JSONL usage fields, replace loop wall-clock proxy) → (4) `env-guard.sh` rebuild — SPEC KNOWN: a hook preventing any agent from READING env-var/secret VALUES; agents may only reference variable names/paths, never the keys themselves.
