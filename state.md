...
- **Session queue (user-approved order):** ✅ (1) persistence insurance → ✅ (2) codebase audit (1 CRITICAL fixed via remove-by-rename, 6 minors, AGENTS #16 promoted; baseline 498 → ✅ ~/CLAUDE.md counter updated to 498/598) → ✅ (3) `--cost` engine SHIPPED (real USD from session JSONL; loop `--budget 5.00` = dollars now; **bats 518**, live: $204 all-time / $41 today) → (4) `env-guard.sh` rebuild — SPEC KNOWN: a hook preventing any agent from READING env-var/secret VALUES; agents may only reference variable names/paths, never the keys themselves. NEXT after (4): roadmap #3 token-limit auto-resume.
