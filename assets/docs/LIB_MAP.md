...
- **`lib/*.sh`** — sourced helpers:
  ...
  - `devsync.sh` — `ac_dev_ensure`: automatic `dev/` provisioning + publication boundary. Writes the boundary to `.git/info/exclude` (NOT `.gitignore` — naming those paths in a committed file is itself a public trace of local tooling), untracks anything the boundary now covers, and syncs the local-only root surface into `dev/context/` so the private `<project>-dev` mirror actually carries it. Runs unattended from `pp` and `bootstrap`; must run BEFORE any staging
