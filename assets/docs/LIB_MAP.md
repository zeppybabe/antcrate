...
- **`lib/*.sh`** — sourced helpers:
  ...
  - `intel.sh` — Anthropic intel tracker; `intel pull [--quiet] [<id>]` / `intel ls [--json]` / `intel ack all|<id>[ <sha>]` / `intel st`; pinned Anthropic-ONLY sources (`~/.antcrate/intel/sources.json`, non-Anthropic host = exit 2), snapshot-on-hash-change + append-only `new.jsonl`/`acked.jsonl`; `intel: N unread` line in `--status`; pairs with `systemd/antcrate-intel.timer` (retrieval); the cognition procedure is folded into the root SKILL.md "Intel review" section (proposals only, never edits — the separate intel skill retired 2026-07-10, recoverable on origin/attic). Spec: `docs/specs/2026-06-10-anthropic-intel-tracker-design.md`
