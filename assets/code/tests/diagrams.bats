...
# ---------- the public diagram must not leak local-only paths (2026-07-25) ----------
#
# tree.mmd is COMMITTED, so it is a published artifact. It was generated by
# walking the filesystem, which meant git-ignored dev context appeared in it by
# name: circular-pong's diagram listed dev/context/CLAUDE.md and
# dev/context/AGENTS.md. Filename-only, but under the AI-trace-free rule a
# filename is exactly the trace that matters.
#
# The fix filters at the DIAGRAM layer, never the ADDRESS layer. Addresses are
# positional (seg=$i in _ac_addr_walk); filtering them would renumber every
# address in the project. So the diagram simply omits ignored nodes and its
# addresses keep gaps — which is correct, because they must still agree with
# what `antcrate addr` resolves.

@test "diagrams: tree omits git-ignored paths from the published diagram" {
    src "ac_registry_upsert proj '$R' scripts ''"
    ( cd "$R" && git init -q -b master && git config user.email t@e.com && git config user.name t )
    printf 'dev/\n/CLAUDE.md\n' > "$R/.gitignore"
    mkdir -p "$R/dev/context"
    echo "ctx"    > "$R/dev/context/CLAUDE.md"
    echo "secret" > "$R/CLAUDE.md"
    echo "public" > "$R/README.md"

    run src "ac_diagrams_tree_to_mermaid proj"
    [ "$status" -eq 0 ]
    [[ "$output" == *"README.md"* ]]      # published content still drawn
    [[ "$output" != *"CLAUDE.md"* ]]
    [[ "$output" != *"context"* ]]
}

@test "diagrams: a non-git directory still renders everything" {
    src "ac_registry_upsert proj '$R' scripts ''"
    echo "public" > "$R/README.md"
    run src "ac_diagrams_tree_to_mermaid proj"
    [ "$status" -eq 0 ]
