...
## 2026-06-12 — chess: docs-as-secrets regime + path repoint; docsguard hook templates SHIPPED — bats 676 → 687

User directive: chess build docs (CLAUDE.md, specs, all markdown) are SECRETS — must never leak. Also user manually moved the project from `~/projects/projects/chess` (scaffold's domain-nested path) to `~/projects/chess/Xhess/`. **(1) Registry repointed** via `ac_registry_set_path` (the sanctioned lib helper; `--repoint` flag still a queued proposal — this was its use case, again). Registered name stays `chess`; only the path changed. **(2) Pre-surgery backup** `chess-20260612T203235Z.tar.gz`. **(3) History rebuilt doc-free:** the approved spec was already in commit 673bde4, but NO remote existed — orphan-branch rebuild, master replaced at `aff7751` (code+scaffold only), reflog expired + gc pruned: zero doc paths reachable anywhere. `.gitignore` gained `*.md` + `docs/` (+ build artifacts) so docs can't stage by accident. **(4) NEW antcrate hook templates** (TDD, 11 bats in `tests/docsguard_templates.bats`): `pre-commit-docsguard` (refuses staged *.md / docs/ paths) and `pre-push-docsguard` (the wire gate — scans every outgoing commit per pushed ref: full history on new refs, remote..local on updates; refuses if any doc path would leave the machine). `pre-push-docsguard` INSTALLED into chess via `--hook-install` (clean slot); `pre-commit-docsguard` NOT installed there — slot already holds pre-commit-secrets and git runs one file per slot → proposal `hook-chain` filed (pre-commit.d dispatcher so guard templates compose). Live-verified: `git add docs CLAUDE.md state.md` refused by gitignore; installed pre-push rc0 on clean history; refusal paths bats-proven. **bats 676 → 687, full `--ci` PASS, commit 57eecff, wrapper reinstalled (templates ship from the installed copy — that's where `--hook-install` resolves them).**

## 2026-06-12 — NEW PROJECT registered: `chess` (domain `projects`) — design spec approved + committed

Brainstormed (superpowers flow, all sections user-approved) and scaffolded via `antcrate --start chess --domain projects --meta cpp,lua,raylib`; bootstrapped (git init + commit 673bde4). Retro 2D chess for Linux: C++20 + raylib + Lua 5.4/sol2, **Approach B** (C++ owns rules/UCI/renderer/save; Lua owns ALL content — menus, unlockable themes, coroutine-scripted celebrations like the checkmate fly-off). Stockfish via UCI as a separate tunable process (light+free; no-engine → hot-seat fallback). v1 includes undo, clocks, PGN, sound. Spec: `~/projects/projects/chess/docs/specs/2026-06-12-chess-game-design.md`. Six build phases, Phase 1 = core rules + perft (TDD). Next: user spec review → writing-plans.
